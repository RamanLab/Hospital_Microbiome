...
# --- List of environment categories ---
env_list <- c("all", "Hospital", "Metro", "Office")

# Optional: Store phyloseq objects in a list instead of global variables
physeq_list <- list()

# --- Loop through each environment and build phyloseq object ---
for (env in env_list) {
  message("Processing environment: ", env)
  
  # Construct file paths safely using file.path
  file_otu  <- file.path(path_phyloseq, paste0(env, "_filtered_data.csv"))
  file_meta <- file.path(path_phyloseq, paste0(env, "_filtered_metadata.csv"))
  file_taxa <- file.path(path_phyloseq, paste0(env, "_filtered_taxa.csv"))

  # Load OTU table and convert to otu_table
  ...
  physeq_obj <- phyloseq(otu_mat, taxa_mat, meta_sample)

  # Option 1: Store in a list (recommended)
  physeq_list[[env]] <- physeq_obj

  # Option 2: Assign to global environment (less safe)
  assign(paste0("physeq_", env), physeq_obj)
}
...
# Save plot
ggsave(paste0(path_figures, "Figure_2B.pdf"), plot = plot_shannon, width = 4,
       height = 5)
...
for (i in 1:100) {
  ...
  alpha_div$Environment <- sampled_meta$Environment  
  alpha_div$Iteration <- i  
...
p.shannon.random <- ggplot(alpha_results, aes(x = Environment, y = diversity_shannon, color = Environment)) +
  ...
  stat_compare_means(method = "anova", label.y = max(alpha_results$diversity_shannon) + 0.2) +  
...
# Saving the plot
ggsave(paste0(path_figures, "Supplementary_Figure_1B.pdf"), plot = p.shannon.random, 
       width = 4, height = 5)
...
write.csv(comp_data, paste0("/mnt/Local_Disk_1/2_Hospital_Microbiome/Data/Output_data/4_class_abundance_data.tsv"), row.names = FALSE)
